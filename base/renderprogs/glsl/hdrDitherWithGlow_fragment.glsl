...
void main() {
	vec2 tCoords = vofi_TexCoord0 . xy ;
	vec2 tCoordsGlow = vofi_TexCoord0 . zw ;
	vec3 glow = tex2D ( samp2 , tCoordsGlow ). xyz * _fa_[0 /* rpDiffuseModifier */] . x ;
	fo_Color = tex2D ( samp0 , tCoords ) + vec4 ( glow , 0.0 ) + tex2D ( samp1 , gl_FragCoord. xy * vec2 ( 0.25 , 0.25 ) ) * vec4 ( 1.0 / 128.0 , 1.0 / 128.0 , 1.0 / 128.0 , 0.0 ) ;
}