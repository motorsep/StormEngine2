...
void main() {
	...
	vec2 origCoord = screenTexCoord ;
	screenTexCoord += ( localNormal * vofi_TexCoord2 . xy ) ;
	screenTexCoord = saturate ( screenTexCoord ) ;
	vec2 altCoord = screenTexCoord - origCoord ;
	altCoord = origCoord - altCoord ;
	fo_Color = tex2D ( samp0 , origCoord. xy ) ;
	vec4 d = tex2D ( samp3 , altCoord. xy ) ;
	if ( d. x > gl_FragCoord . z )
	{
		fo_Color = tex2D ( samp0 , altCoord. xy ) ;
	}
	d = tex2D ( samp3 , screenTexCoord. xy ) ;
	if ( d. x > gl_FragCoord . z )
	{
		fo_Color = tex2D ( samp0 , screenTexCoord. xy ) ;
	}
}