...
void main() {
	vec4 bump = tex2D ( samp0 , vofi_TexCoord0 ) * 2.0 - 1.0 ;
	vec3 localNormal = vec3 ( bump. wy , 0.0 ) ;
	localNormal. z = sqrt ( 1.0 - dot3 ( localNormal , localNormal ) ) ;
	float
	dotVN = saturate ( dot3 ( localNormal , normalize ( vofi_TexCoord1 ) ) ) ;
	float
	fresnelVal = pow ( 1.0 - dotVN , vofi_TexCoord2 . x ) * vofi_TexCoord2 . y + vofi_TexCoord2 . z ;
	vec4
	outColor = vec4 ( fresnelVal ) ;
	outColor. a = 1 ;
	fo_Color = tex2D ( samp1 , vofi_TexCoord0 ) * outColor * vofi_TexCoord3 * vofi_Color ;
}