...
void main() {
	...
	vec4 worldPosition ;
	worldPosition. x = dot4 ( modelPosition , _va_[18 /* rpModelMatrixX */] ) ;
	worldPosition. y = dot4 ( modelPosition , _va_[19 /* rpModelMatrixY */] ) ;
	worldPosition. z = dot4 ( modelPosition , _va_[20 /* rpModelMatrixZ */] ) ;
	worldPosition. w = dot4 ( modelPosition , _va_[21 /* rpModelMatrixW */] ) ;
	vec4 toLightGlobal = _va_[26 /* rpGlobalLightOrigin */] - worldPosition ;
	vofi_TexCoord8 = toLightGlobal ;
	vec4 viewPosition ;
	viewPosition. x = dot4 ( modelPosition , _va_[22 /* rpModelViewMatrixX */] ) ;
	viewPosition. y = dot4 ( modelPosition , _va_[23 /* rpModelViewMatrixY */] ) ;
	viewPosition. z = dot4 ( modelPosition , _va_[24 /* rpModelViewMatrixZ */] ) ;
	viewPosition. w = dot4 ( modelPosition , _va_[25 /* rpModelViewMatrixW */] ) ;
	vofi_TexCoord9 = viewPosition ;
	vofi_TexCoord10 = defaultTexCoord ;
	vofi_TexCoord10 . x = dot4 ( in_TexCoord . xy , _va_[27 /* rpGlossMatrixS */] ) ;
	vofi_TexCoord10 . y = dot4 ( in_TexCoord . xy , _va_[28 /* rpGlossMatrixT */] ) ;
	vofi_Color = saturate ( ( swizzleColor ( in_Color ) * _va_[12 /* rpVertexColorModulate */] ) + _va_[13 /* rpVertexColorAdd */] ) ;
}