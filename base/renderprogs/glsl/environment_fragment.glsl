...
const vec4 matrixCoCg1YtoRGB1XFull = vec4( 1.0, -1.0, 0.0, 1.0 );
const vec4 matrixCoCg1YtoRGB1YFull = vec4( 0.0, 1.0, -0.50196078, 1.0 );
const vec4 matrixCoCg1YtoRGB1ZFull = vec4( -1.0, -1.0, 1.00392156, 1.0 );
vec4 ConvertYCoCgToRGBFull (vec4 YCoCg ) {
	vec4 rgbColor ;
...
void main() {
	...
	vec4 envMap = ConvertYCoCgToRGBFull ( texCUBE ( samp0 , reflectionVector ) ) ;
	fo_Color = vec4 ( envMap. xyz , 1.0 ) * vofi_Color ;
}