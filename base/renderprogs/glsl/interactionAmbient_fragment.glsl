...
const float ambientScale = 0.4;
const float ambientBias = 0.6;
...
void main() {
	...
	float vDotN = saturate ( dot3 ( normalize ( vofi_TexCoord6 . xyz ) , localNormal ) ) ;
	vec3 diffuseColor = diffuseMap * _fa_[0 /* rpDiffuseModifier */] . xyz ;
	float ambientShade = vDotN * ambientScale + ambientBias ;
	vec3 lightColor = ambientShade * lightProj. xyz * lightFalloff. xyz ;
	fo_Color . xyz = ( diffuseColor ) * lightColor * vofi_Color . xyz ;
	fo_Color . w = 1.0 ;
}