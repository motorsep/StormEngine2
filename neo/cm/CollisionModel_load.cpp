...
cmHandle_t idCollisionModelManagerLocal::SetupTrmModel( const idTraceModel& trm, const idMaterial* material )
{
	...
	for( i = 0; i < trm.numPolys; i++, trmPoly++ )
	{
		...
		poly->contents = material->GetContentFlags();
...
cm_model_t* idCollisionModelManagerLocal::CollisionModelForMapEntityDmap( const idDmapMapEntity* mapEnt )
{
	...
	CM_EstimateVertsAndEdgesDmap( mapEnt, &model->maxVertices, &model->maxEdges );
	model->numVertices = 0;
	model->numEdges = 0;
	model->vertices = ( cm_vertex_t* ) Mem_ClearedAlloc( model->maxVertices * sizeof( cm_vertex_t ), TAG_COLLISION );
	model->edges = ( cm_edge_t* ) Mem_ClearedAlloc( model->maxEdges * sizeof( cm_edge_t ), TAG_COLLISION );
...
void idCollisionModelManagerLocal::BuildModelsDmap( const idDmapMapFile* mapFile )
{
	...
	if( !LoadCollisionModelFile( mapFile->GetName(), mapFile->GetGeometryCRC() ) )
	{
		...
		for( i = 0; i < mapFile->GetNumEntities(); i++ )
		{
			...
			models[numModels] = CollisionModelForMapEntityDmap( mapEnt );
			if( models[ numModels] )
			{
				numModels++;
			}
