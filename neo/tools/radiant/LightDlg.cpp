...
void CLightInfo::ToDictWriteAllInfo( idDict *e ) {
	...
	if (strTexture.GetLength() > 0 ) {
		...
		// foresthale 2014-05-21: make sure we always set all dict keys, or it becomes impossible to remove a key using the light editor
		e->Set("texture", "");
	}
...
void CLightInfo::ToDict( idDict *e ) {
	...
	// foresthale 2014-05-27: apparently radiant applies these keys differently, so it's important to use the old code in radiant
	...
	{
		// foresthale 2014-05-21: fix for the "Projected Light won't go back to Point Light" bug in the light editor - we have to clear all the keys that can be set but were not, these were being Delete(x)'d but we need to Set(x,"") instead
		e->Set("noshadows", "");
		e->Set("nospecular", "");
		e->Set("nodiffuse", "");
		e->Set("falloff", "");
