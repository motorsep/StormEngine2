	...
	if (patch) {
		...
		for ( i = 0; i < patch->width; i++ ) {
			for ( j = 0; j < patch->height; j++ ) {
				...
				patch->ctrl(i, j).SetTexCoordS(f->brushprimit_texdef.coords[0][0] * x + f->brushprimit_texdef.coords[0][1] * y + f->brushprimit_texdef.coords[0][2]);
				patch->ctrl(i, j).SetTexCoordT(f->brushprimit_texdef.coords[1][0] * x + f->brushprimit_texdef.coords[1][1] * y + f->brushprimit_texdef.coords[1][2]);
			}
		}
	}
}
