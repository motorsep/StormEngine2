...
void Dmap_f( const idCmdArgs &args ) {
	...
	// foresthale 2014-05-15: dmap has a habit of unloading sound decls, and this causes the game to load them in the next map that is played, which causes a fatal error because it isn't the main thread, so we have to force off threading for the rest of the session to prevent fatal errors
	cvarSystem->SetCVarBool( "com_smp", false );
	Dmap( args );
	com_editors &= ~EDITOR_DMAP;
