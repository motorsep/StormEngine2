...
static uint	shadowIndexes[MAX_SHADOW_INDEXES];
...
===================
*/
static void R_ProjectPointsToFarPlane(const idDmapRenderEntityLocal *ent, const idDmapRenderLightLocal *light,
...
=================
*/
static void R_AddSilEdges(const srfDmapTriangles_t *tri, unsigned short *pointCull, const idPlane frustum[6]) {
	int		v1, v2;
	int		i;
	silEdge_t	*sil;
	int		numPlanes;
...
================
*/
static void R_CalcPointCull(const srfDmapTriangles_t *tri, const idPlane frustum[6], unsigned short *pointCull) {
	int i;
	int frontBits;
	float *planeSide;
	byte *side1, *side2;

	dmapSIMDProcessor->Memset(remap, -1, tri->numVerts * sizeof(remap[0]));
	...
	dmapSIMDProcessor->Memset(side1, 0, tri->numVerts * sizeof(byte));
	dmapSIMDProcessor->Memset(side2, 0, tri->numVerts * sizeof(byte));
	...
	for (i = 0; i < 6; i++) {
		...
		dmapSIMDProcessor->Dot(planeSide, frustum[i], tri->verts, tri->numVerts);
		dmapSIMDProcessor->CmpLT(side1, i, planeSide, LIGHT_CLIP_EPSILON, tri->numVerts);
		dmapSIMDProcessor->CmpGT(side2, i, planeSide, -LIGHT_CLIP_EPSILON, tri->numVerts);
	}
	for (i = 0; i < tri->numVerts; i++) {
...
=================
*/
static void R_CreateShadowVolumeInFrustum(const idDmapRenderEntityLocal *ent,
	const srfDmapTriangles_t *tri,
	const idDmapRenderLightLocal *light,
...
===================
*/
void R_MakeShadowFrustums(idDmapRenderLightLocal *light) {
	int		i, j;
...
srfDmapTriangles_t *R_CreateShadowVolume(const idDmapRenderEntityLocal *ent,
	...
	dmap_tr.pc.c_createShadowVolumes++;
	...
	dmapSIMDProcessor->Memcpy(newTri->shadowVertexes, shadowVerts, newTri->numVerts * sizeof(newTri->shadowVertexes[0]));
	...
	if (1 /* sortCapIndexes */) {
		...
		for (i = 0; i < indexFrustumNumber; i++) {
			...
			dmapSIMDProcessor->Memcpy(newTri->indexes + newTri->numShadowIndexesNoCaps,
		...
		for (i = 0; i < indexFrustumNumber; i++) {
			...
			dmapSIMDProcessor->Memcpy(newTri->indexes + newTri->numShadowIndexesNoFrontCaps,
		...
		for (i = 0; i < indexFrustumNumber; i++) {
			...
			dmapSIMDProcessor->Memcpy(newTri->indexes + newTri->numIndexes,
		...
		dmapSIMDProcessor->Memcpy(newTri->indexes, shadowIndexes, newTri->numIndexes * sizeof(newTri->indexes[0]));
	}
