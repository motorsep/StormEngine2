...
/*
===================
idDmapRenderWorldLocal::ScreenRectForWinding
===================
*/
idScreenRect idDmapRenderWorldLocal::ScreenRectFromWinding(const idWinding *w, dmapViewEntity_t *space) {
	idScreenRect	r;
	int				i;
	idVec3			v;
	idVec3			ndc;
	...
	for (i = 0; i < w->GetNumPoints(); i++) {
		...
		windowX = 0.5f * (1.0f + ndc[0]) * (dmap_tr.viewDef->viewport.x2 - dmap_tr.viewDef->viewport.x1);
		windowY = 0.5f * (1.0f + ndc[1]) * (dmap_tr.viewDef->viewport.y2 - dmap_tr.viewDef->viewport.y1);
...
===================
*/
bool idDmapRenderWorldLocal::PortalIsFoggedOut(const dmapPortal_t *p) {
	idDmapRenderLightLocal	*ldef;
	const idWinding	*w;
	int			i;
	idPlane		forward;
	...
	lightShader->EvaluateRegisters(regs, ldef->parms.shaderParms, dmap_tr.viewDef->renderView.shaderParms, dmap_tr.viewDef->renderView.time * 0.001f, ldef->parms.referenceSound);
	...
	forward[0] = a * dmap_tr.viewDef->worldSpace.modelViewMatrix[2];
	forward[1] = a * dmap_tr.viewDef->worldSpace.modelViewMatrix[6];
	forward[2] = a * dmap_tr.viewDef->worldSpace.modelViewMatrix[10];
	forward[3] = a * dmap_tr.viewDef->worldSpace.modelViewMatrix[14];
...
===================
*/
void idDmapRenderWorldLocal::FloodViewThroughArea_r(const idVec3 origin, int areaNum,
	const struct portalStack_s *ps) {
	dmapPortal_t*		p;
	float			d;
	dmapPortalArea_t *	area;
	const dmapPortalStack_t	*check;
	dmapPortalStack_t	newStack;
	int				i, j;
	idVec3			v1, v2;
	int				addPlanes;
	...
	for (p = area->portals; p; p = p->next) {
		...
		newStack.rect = ScreenRectFromWinding(&w, &dmap_tr.identitySpace);
...
=======================
*/
void idDmapRenderWorldLocal::FlowViewThroughPortals(const idVec3 origin, int numPlanes, const idPlane *planes) {
	dmapPortalStack_t	ps;
	int				i;
	...
	ps.rect = dmap_tr.viewDef->scissor;

	if (dmap_tr.viewDef->areaNum < 0){

		for (i = 0; i < numPortalAreas; i++) {
			areaScreenRect[i] = dmap_tr.viewDef->scissor;
		}
		...
		FloodViewThroughArea_r(origin, dmap_tr.viewDef->areaNum, &ps);
	}
}
...
===================
*/
void idDmapRenderWorldLocal::FloodLightThroughArea_r(idDmapRenderLightLocal *light, int areaNum,
	const struct portalStack_s *ps) {
	dmapPortal_t*		p;
	float			d;
	dmapPortalArea_t *	area;
	const dmapPortalStack_t	*check, *firstPortalStack = NULL;
	dmapPortalStack_t	newStack;
	int				i, j;
	idVec3			v1, v2;
	int				addPlanes;
...
=======================
*/
void idDmapRenderWorldLocal::FlowLightThroughPortals(idDmapRenderLightLocal *light) {
	dmapPortalStack_t	ps;
	int				i;
...
/*
===================
idDmapRenderWorldLocal::FloodFrustumAreas_r
===================
*/
areaNumRef_t *idDmapRenderWorldLocal::FloodFrustumAreas_r(const idFrustum &frustum, const int areaNum, const idBounds &bounds, areaNumRef_t *areas) {
	dmapPortal_t *p;
	dmapPortalArea_t *portalArea;
	idBounds newBounds;
	areaNumRef_t *a;
...
/*
===================
idDmapRenderWorldLocal::FloodFrustumAreas
...
===================
*/
areaNumRef_t *idDmapRenderWorldLocal::FloodFrustumAreas(const idFrustum &frustum, areaNumRef_t *areas) {
	idBounds bounds;
	areaNumRef_t *a;
...
================
*/
bool idDmapRenderWorldLocal::CullEntityByPortals(const idDmapRenderEntityLocal *entity, const dmapPortalStack_t *ps) {
...
===================
*/
void idDmapRenderWorldLocal::AddAreaEntityRefs(int areaNum, const dmapPortalStack_t *ps) {
	dmapAreaReference_t		*ref;
	idDmapRenderEntityLocal	*entity;
	dmapPortalArea_t		*area;
	dmapViewEntity_t		*vEnt;
	idBounds			b;
	...
	for (ref = area->entityRefs.areaNext; ref != &area->entityRefs; ref = ref->areaNext) {
		...
		R_FreeEntityDefFadedDecalsDmap(entity, dmap_tr.viewDef->renderView.time);

		// check for completely suppressing the model
		if (!r_skipSuppress.GetBool()) {
			if (entity->parms.suppressSurfaceInViewID
				&& entity->parms.suppressSurfaceInViewID == dmap_tr.viewDef->renderView.viewID) {
				continue;
			}
			if (entity->parms.allowSurfaceInViewID
				&& entity->parms.allowSurfaceInViewID != dmap_tr.viewDef->renderView.viewID) {
				continue;
			}
		}
		...
		vEnt = R_SetEntityDefViewEntityDmap(entity);
...
================
*/
bool idDmapRenderWorldLocal::CullLightByPortals(const idDmapRenderLightLocal *light, const dmapPortalStack_t *ps) {
	int				i, j;
	...
	if (r_useLightCulling.GetInteger() >= 2) {
		...
		for (i = 0; i < 6; i++) {
			...
			if (light->frustum[i].Distance(dmap_tr.viewDef->renderView.vieworg) >= 0) {
				continue;
			}
...
===================
*/
void idDmapRenderWorldLocal::AddAreaLightRefs(int areaNum, const dmapPortalStack_t *ps) {
	...
	for (lref = area->lightRefs.areaNext; lref != &area->lightRefs; lref = lref->areaNext) {
		...
		if (r_useLightCulling.GetInteger() >= 3 &&
			...
			&& light->areaNum != -1 && !dmap_tr.viewDef->connectedAreas[light->areaNum]) {
			continue;
		}
		...
		vLight = R_SetLightDefViewLightDmap(light);
...
===================
*/
void idDmapRenderWorldLocal::AddAreaRefs(int areaNum, const dmapPortalStack_t *ps) {
	// mark the viewCount, so r_showPortals can display the
	// considered portals
	portalAreas[areaNum].viewCount = dmap_tr.viewCount;
...
===================
*/
void idDmapRenderWorldLocal::BuildConnectedAreas_r(int areaNum) {
	dmapPortalArea_t	*area;
	dmapPortal_t		*portal;

	if (dmap_tr.viewDef->connectedAreas[areaNum]) {
		return;
	}

	dmap_tr.viewDef->connectedAreas[areaNum] = true;
...
===================
*/
void idDmapRenderWorldLocal::BuildConnectedAreas(void) {
	int		i;

	dmap_tr.viewDef->connectedAreas = (bool *)R_FrameAlloc(numPortalAreas
		* sizeof(dmap_tr.viewDef->connectedAreas[0]));

	// if we are outside the world, we can see all areas
	if (dmap_tr.viewDef->areaNum == -1) {
		for (i = 0; i < numPortalAreas; i++) {
			dmap_tr.viewDef->connectedAreas[i] = true;
		}
		return;
	}

	// start with none visible, and flood fill from the current area
	memset(dmap_tr.viewDef->connectedAreas, 0, numPortalAreas * sizeof(dmap_tr.viewDef->connectedAreas[0]));
	BuildConnectedAreas_r(dmap_tr.viewDef->areaNum);
}
...
=============
*/
void idDmapRenderWorldLocal::FindViewLightsAndEntities(void) {
	// clear the visible lightDef and entityDef lists
	dmap_tr.viewDef->viewLights = NULL;
	dmap_tr.viewDef->viewEntitys = NULL;

	// find the area to start the portal flooding in
	if (!r_usePortals.GetBool()) {
		// debug tool to force no portal culling
		dmap_tr.viewDef->areaNum = -1;
	}
	else {
		dmap_tr.viewDef->areaNum = PointInArea(dmap_tr.viewDef->initialViewAreaOrigin);
	}
	...
	if (r_singleArea.GetBool()) {
		...
		if (dmap_tr.viewDef->areaNum >= 0) {
			...
			if (dmap_tr.viewDef->areaNum != lastPrintedAreaNum) {
				lastPrintedAreaNum = dmap_tr.viewDef->areaNum;
				common->Printf("entering portal area %i\n", dmap_tr.viewDef->areaNum);
			}

			for (i = 0; i < 5; i++) {
				ps.portalPlanes[i] = dmap_tr.viewDef->frustum[i];
			}
			ps.numPortalPlanes = 5;
			ps.rect = dmap_tr.viewDef->scissor;

			AddAreaRefs(dmap_tr.viewDef->areaNum, &ps);
		}
	}
	else {
		...
		FlowViewThroughPortals(dmap_tr.viewDef->renderView.vieworg, 5, dmap_tr.viewDef->frustum);
	}
}
...
==============
*/
int idDmapRenderWorldLocal::NumPortals(void) const {
...
==============
*/
qhandle_t idDmapRenderWorldLocal::FindPortal(const idBounds &b) const {
	int				i, j;
	idBounds		wb;
	dmapDoublePortal_t	*portal;
	idWinding		*w;
...
=============
*/
void	idDmapRenderWorldLocal::FloodConnectedAreas(dmapPortalArea_t *area, int portalAttributeIndex) {
	...
	for (dmapPortal_t *p = area->portals; p; p = p->next) {
...
==============
*/
bool	idDmapRenderWorldLocal::AreasAreConnected(int areaNum1, int areaNum2, portalConnection_t connection) {
	if (areaNum1 == -1 || areaNum2 == -1) {
		return false;
	}
	if (areaNum1 > numPortalAreas || areaNum2 > numPortalAreas || areaNum1 < 0 || areaNum2 < 0) {
		common->Error("idDmapRenderWorldLocal::AreAreasConnected: bad parms: %i, %i", areaNum1, areaNum2);
	}
	...
	if (attribute >= NUM_PORTAL_ATTRIBUTES || (1 << attribute) != (int)connection) {
		common->Error("idDmapRenderWorldLocal::AreasAreConnected: bad connection number: %i\n", (int)connection);
	}
...
==============
*/
void		idDmapRenderWorldLocal::SetPortalState(qhandle_t portal, int blockTypes) {
	if (portal == 0) {
		return;
	}
...
==============
*/
int		idDmapRenderWorldLocal::GetPortalState(qhandle_t portal) {
	if (portal == 0) {
		return 0;
	}
...
/*
=====================
idDmapRenderWorldLocal::ShowPortals

Debugging tool, won't work correctly with SMP or when mirrors are present
=====================
*/
/* void idDmapRenderWorldLocal::ShowPortals() {
	int			i, j;
	dmapPortalArea_t	*area;
	dmapPortal_t	*p;
	idWinding	*w;
	...
	for (i = 0; i < numPortalAreas; i++) {
		...
		for (p = area->portals; p; p = p->next) {
			...
			if (portalAreas[p->intoArea].viewCount != dmap_tr.viewCount) {
				// red = can't see
				qglColor3f(1, 0, 0);
			}
			else {
