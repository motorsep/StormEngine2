...
/*
=================
idDmapRenderModelMD3::InitFromFile
=================
*/
void idDmapRenderModelMD3::InitFromFile(const char *fileName) {
	int					i, j;
	...
	dataSize += size;
	md3 = (md3Header_t *)Mem_Alloc(size, TAG_DMAP);
...
/*
=================
idDmapRenderModelMD3::IsDynamicModel
=================
*/
dynamicModel_t idDmapRenderModelMD3::IsDynamicModel() const {
	return DM_CACHED;
}

/*
=================
idDmapRenderModelMD3::LerpMeshVertexes
=================
*/
void idDmapRenderModelMD3::LerpMeshVertexes(srfDmapTriangles_t *tri, const struct md3Surface_s *surf, const float backlerp, const int frame, const int oldframe) const {
	short	*oldXyz, *newXyz;
	...
	if (backlerp == 0) {
		...
		for (vertNum = 0; vertNum < numVerts; vertNum++, newXyz += 4) {
			...
			idDmapDrawVert *outvert = &tri->verts[tri->numVerts];
...
/*
=============
idDmapRenderModelMD3::InstantiateDynamicModel
=============
*/
idDmapRenderModel *idDmapRenderModelMD3::InstantiateDynamicModel(const struct dmapRenderEntity_s *ent, const struct dmapViewDef_s *view, idDmapRenderModel *cachedModel) {
	int				i, j;
	float			backlerp;
	int *			triangles;
	float *			texCoords;
	...
	for (i = 0; i < md3->numSurfaces; i++) {
		...
		R_AllocStaticTriSurfVertsDmap(tri, surface->numVerts);
		R_AllocStaticTriSurfIndexesDmap(tri, surface->numTriangles * 3);
		tri->bounds.Clear();
		...
		for (j = 0; j < numVerts; j++) {
			idDmapDrawVert *stri = &tri->verts[j];
...
/*
=====================
idDmapRenderModelMD3::Bounds
=====================
*/

idBounds idDmapRenderModelMD3::Bounds(const struct dmapRenderEntity_s *ent) const {
	idBounds		ret;
