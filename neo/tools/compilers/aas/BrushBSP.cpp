...
============
*/
idBrushBSP::idBrushBSP( const int bspGridSize )
	: BSP_GRID_SIZE( bspGridSize ) {
	root = outside = NULL;
...
void idBrushBSP::Build( idBrushList brushList, int skipContents,
	...
	{
		// estimate the grid usage and reserve some space to reduce thrashing
		idVec3 treeSize = treeBounds[ 1 ] - treeBounds[ 0 ];
		treeSize /= BSP_GRID_SIZE;
		
		gridCells.AssureSize( treeSize.x * treeSize.y * treeSize.z );
		gridCells.AssureSize( 0 );
	}
...
void idBrushBSP::LeakFile( const idStr &fileName ) {
	...
	lineFile = fileSystem->OpenFileWrite( qpath, "fs_basepath" );
	if ( !lineFile ) {
...
void idBrushBSP::MeltLeafNodePortals( idBrushBSPNode *node, int skipContents, idVectorSet<idVec3,3> &vertexList ) {
	...
	for ( p1 = node->GetPortals(); p1; p1 = p1->Next(s1) ) {
		...
		for ( i = 0; i < vertexList.Num(); i++ ) {
			if ( p1->winding->InsertPointIfOnEdge( (const idVec5&)vertexList[i], p1->plane, 0.1f ) ) {
				numInsertedPoints++;
			}
		}
	}
