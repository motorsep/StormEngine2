...
idCVar r_skipInk( "r_skipInk", "0", CVAR_RENDERER | CVAR_BOOL, "bypasses all shader passes that use _currentDepth" );
...
idCVar r_skipROQ( "r_skipROQ", "0", CVAR_RENDERER | CVAR_BOOL, "skip ROQ videoS" );

// visual debugging info
idCVar r_showNvidiaHack( "r_showNvidiaHack", "1", CVAR_RENDERER | CVAR_BOOL, "use shaders to color wireframes on Nvidia hardware and non-shader for AMD (might be broken)" );
...
idCVar r_shadowMapImageSize( "r_shadowMapImageSize", "1024", CVAR_RENDERER | CVAR_INTEGER | CVAR_ARCHIVE, "", 128, 8192 );
idCVar r_shadowMapJitterScale( "r_shadowMapJitterScale", "2.25", CVAR_RENDERER | CVAR_FLOAT, "scale factor for jitter offset (sharpness; diameter of possion disk)" );
idCVar r_shadowMapBiasScale( "r_shadowMapBiasScale", "0.0001", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_FLOAT, "scale factor for jitter bias" );
...
idCVar r_shadowMapQuality( "r_shadowMapQuality", "3", CVAR_RENDERER | CVAR_INTEGER | CVAR_ARCHIVE, "0 = 8bit, 1 = 16bit, 2 = 24bit, 3 = 32bit, 4 = 32bit float" );
idCVar r_shadowMapCascadeScale( "r_shadowMapCascadeScale", "0.1", CVAR_RENDERER | CVAR_FLOAT, "makes parallel lights influence farther" );
...
static void R_CheckPortableExtensions()
{
	...
	else if( glConfig.textureCompressionAvailable )
	{
	...
	if( glConfig.glVersion >= 3.1f )
	{
		...
		qglProgramStringARB = ( PFNGLPROGRAMSTRINGARBPROC )GLimp_ExtensionPointer( "glProgramString" );
		qglBindProgramARB = ( PFNGLBINDPROGRAMARBPROC )GLimp_ExtensionPointer( "glBindProgram" );
		qglGenProgramsARB = ( PFNGLGENPROGRAMSARBPROC )GLimp_ExtensionPointer( "glGenPrograms" );
...
void R_ReadTiledPixels( int width, int height, byte* buffer, renderView_t* ref = NULL )
{
	...
	// discard anything currently on the list
	//tr.SwapCommandBuffers( NULL, NULL, NULL, NULL );
	// override the render resolution so we can take our screenshots
//	tr.screenshotOverrideWidth = sysWidth;
//	tr.screenshotOverrideHeight = sysHeight;
	...
	//tr.SwapCommandBuffers( NULL, NULL, NULL, NULL );
	// undo our override
//	tr.screenshotOverrideWidth = 0;
//	tr.screenshotOverrideHeight = 0;
...
void R_EnvShot_f( const idCmdArgs &args )
{
	...
	for ( i = 0 ; i < 6 ; i++ )
	{
		...
		ref.width = glConfig.nativeScreenWidth;
		ref.height = glConfig.nativeScreenHeight;
		ref.viewaxis = axis[i];
