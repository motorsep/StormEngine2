...
{
	const unsigned int block_count = ( ( height + 3 ) >> 2 ) * ( ( width + 3 ) >> 2 );
	unsigned int i;
	ALIGN16( byte block[64] );
	byte *p;
	int x, y;
	...
	{
		commonLocal.LoadPacifierBinarizeProgressIncrement(16);
		// FIXME: NVIDIA_7X_HARDWARE_BUG_FIX?
		x = ( i % ( ( width + 3 ) >> 2 ) ) << 2;
		y = ( i / ( ( width + 3 ) >> 2 ) ) << 2;
		p = outBuf + BLOCK_OFFSET( x, y, width, 16 );
		ExtractBlockGimpDDS( inBuf, x, y, width, height, block );
		EncodeAlphaBlockBC3GimpDDS( p, block, 0 );
		EncodeYCoCgBlockGimpDDS( p + 8, block );
	}
