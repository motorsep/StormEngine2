...
idCVar r_glowEnable( "r_glowEnable", "1", CVAR_RENDERER | CVAR_BOOL | CVAR_ARCHIVE, "enables Bloom effect for materials that use a glow pass" );
...
static void RB_PrepareStageTexturing( const shaderStage_t* pStage,  const drawSurf_t* surf )
{
	...
	{
		// motorsep 05-18-2015; set shader var to determine what colorspace to use
	...
	else if ( pStage->texture.texgen == TG_SCRIPTSKY_CUBE )
	{
		...
		transform[ 0 * 4 + 2 ] = skyMat[ 0 ].z;
		transform[ 1 * 4 + 0 ] = skyMat[ 1 ].x;
		transform[ 1 * 4 + 1 ] = skyMat[ 1 ].y;
		transform[ 1 * 4 + 2 ] = skyMat[ 1 ].z;
...
const int INTERACTION_TEXUNIT_GLOSS			= 7;
...
static void RB_SetupForFastPathInteractions( const idVec4& diffuseColor, const idVec4& specularColor)
{
	...
	// gloss matrix
	SetVertexParm( RENDERPARM_GLOSSMATRIX_S, sMatrix.ToFloatPtr() );
	SetVertexParm( RENDERPARM_GLOSSMATRIX_T, tMatrix.ToFloatPtr() );
...
static void RB_RenderInteractions( const drawSurf_t* surfList, const viewLight_t* vLight, int depthFunc, bool performStencilTest, bool useLightDepthBounds )
{
	...
	for( int lightStageNum = 0; lightStageNum < lightShader->GetNumStages(); lightStageNum++ )
	{
		...
		const idVec4 glossColor = lightColor * 2.0f;
		...
		for( int sortedSurfNum = 0; sortedSurfNum < allSurfaces.Num(); sortedSurfNum++ )
		{
			...
			bool useShadowmaps = false; // foresthale 2014-10-26: more consistent branching to fix an ambientLight conflict in the code below
			...
			{
				useShadowmaps = r_useShadowMapping.GetBool() && vLight->globalShadows && !r_skipShadows.GetBool();
				if( useShadowmaps )
				{
			...
			for( int surfaceStageNum = 0; surfaceStageNum < surfaceShader->GetNumStages(); surfaceStageNum++ )
			{
				...
				switch( surfaceStage->lighting )
				{
					...
					case SL_GLOSS:
					{
						...
						inter.glossImage = surfaceStage->texture.image;
						RB_SetupInteractionStage( surfaceStage, surfaceRegs, glossColor.ToFloatPtr(),
												  inter.glossMatrix, inter.glossColor.ToFloatPtr() );
						break;
					}
				}
			}
...
static void RB_ShadowMapPass( const drawSurf_t* drawSurfs, const viewLight_t* vLight, int side )
{
	...
	if( vLight->parallel && side >= 0 )
	{
		...
		float width = cropBounds[1][0] - cropBounds[0][0];
		float height = cropBounds[1][1] - cropBounds[0][1];
		width *= r_shadowMapCascadeScale.GetFloat();
		height *= r_shadowMapCascadeScale.GetFloat();
		cropBounds[0][0] -= width;
	...
	if( side < 0 )
	{
		globalFramebuffers->shadowMapFramebuffer[vLight->shadowLOD]->Bind( 0 );
	}
	else
	{
	...
	for( const drawSurf_t* drawSurf = drawSurfs; drawSurf != NULL; drawSurf = drawSurf->nextOnLight )
	{
		...
		if( drawSurf->space != backEnd.currentSpace )
		{
			...
			{
				// skip object if the object can't be viewed by this side of the point light
				if( idRenderMatrix::CullBoundsToMVP( clipMVP, drawSurf->space->entityDef->localReferenceBounds ) )
				{
					continue;
				}
				RB_SetMVP( clipMVP );
...
=====================
*/
// foresthale 20140403: modified to have glowStage code for our glowbuffer (bloom from glowing materials) rendering
static int RB_DrawShaderPasses( const drawSurf_t* const* const drawSurfs, const int numDrawSurfs,
								const float guiStereoScreenOffset, const int stereoEye, const int stopSort, const bool skipNoBlur, const bool glowStage )	// sikk - Added 'skipNoBlur' arg - No Motionblur Material Stage Flag
{
...
// <--- sikk - Added - No Motionblur Material Stage Flag
			...
			// foresthale 20140403: glowStage
			if( glowStage != pStage->glowStage )
			{
				continue;
			}
...
==================
*/
static void RB_FogPass( const drawSurf_t* drawSurfs,  const drawSurf_t* drawSurfs2, const viewLight_t* vLight, bool glowStage )
{
	renderLog.OpenBlock( vLight->lightShader->GetName() );
...
static void RB_FogAllLights(bool glowStage)
{
	...
	for( viewLight_t* vLight = backEnd.viewDef->viewLights; vLight != NULL; vLight = vLight->next )
	{
		if( vLight->lightShader->IsFogLight() )
		{
			RB_FogPass( vLight->globalInteractions, vLight->localInteractions, vLight, glowStage );
		}
		else if( vLight->lightShader->IsBlendLight() )
		{
			RB_BlendLight( vLight->globalInteractions, vLight->localInteractions, vLight, glowStage );
		}
	}
...
==================
*/
void RB_DrawViewInternal( const viewDef_t* viewDef, const int stereoEye, const bool skipNoBlur )	// sikk - Added 'skipNoBlur' arg - No Motionblur Material Stage Flag
{
	...
	//-------------------------------------------------
	// foresthale 2014-04-25: stop at the first transparent surface - we'll render those after fog
	int processed = 0;
...
void RB_PostProcess( const void* data )
{
	...
	if ( backEnd.glowRenderCopied )
	{
		...
		glowParm[3] = 0.0f; // unused
		// we reuse these outdated renderparms to pass the glow info
		SetFragmentParm( RENDERPARM_LIGHTPROJECTION_S, glowTextureParm ); // rpLightProjectionS
	...
	{
		// use the copy of the system framebuffer
		globalImages->currentRenderImage->Bind();
		screenCorrectionParm[0] = 1.0f;
		screenCorrectionParm[1] = 1.0f;
		screenCorrectionParm[2] = 0.0f;
...
void RB_PostProcessHDRGlowProcess(int textureSizes[4][6])
{
	...
	if ( r_useHDR.GetBool() && !( com_editors ) )
	{
		...
		textureSizes[0][2] = globalImages->glowFramebufferImage16[0]->GetUploadWidth();
		textureSizes[0][3] = globalImages->glowFramebufferImage16[0]->GetUploadHeight();
		textureSizes[0][4] = 0;
		textureSizes[0][5] = 0;
	}
	else
	{
		textureSizes[0][0] = globalImages->glowFramebufferImage8[0]->GetUploadWidth();
		textureSizes[0][1] = globalImages->glowFramebufferImage8[0]->GetUploadHeight();
		textureSizes[0][2] = globalImages->glowFramebufferImage8[0]->GetUploadWidth();
		textureSizes[0][3] = globalImages->glowFramebufferImage8[0]->GetUploadHeight();
		textureSizes[0][4] = 0;
		textureSizes[0][5] = 0;
	}
	...
	textureSizes[1][5] = 0;
	// same size
	textureSizes[2][0] = glConfig.nativeScreenWidth;
	textureSizes[2][1] = glConfig.nativeScreenHeight;
	textureSizes[2][2] = textureSizes[1][2];
	textureSizes[2][3] = textureSizes[1][3];
	textureSizes[2][4] = 0;
	textureSizes[2][5] = 0;
	// same size
	textureSizes[3][0] = glConfig.nativeScreenWidth;
	textureSizes[3][1] = glConfig.nativeScreenHeight;
	textureSizes[3][2] = textureSizes[2][2];
	...
	if ( r_useHDR.GetBool() && !( com_editors ) )
	{
		for (int i = 1;i < 4;i++)
		{
			 ...
			 ||  globalImages->glowFramebufferImage16[i]->GetUploadHeight() != textureSizes[i][1] )
			{
				...
				// we have to purge the fbo also because the original textures no longer exist, it will be recreated
				globalFramebuffers->glowFramebuffer16[i]->PurgeFramebuffer();
			}
		}
	}
	else
	{
		for (int i = 1;i < 4;i++)
		{
			 ...
			 ||  globalImages->glowFramebufferImage8[i]->GetUploadHeight() != textureSizes[i][1] )
			{
				...
				// we have to purge the fbo also because the original textures no longer exist, it will be recreated
				globalFramebuffers->glowFramebuffer8[i]->PurgeFramebuffer();
			}
		}
	}
	...
	for (int curBuffer = 0;curBuffer < 3;curBuffer++)
	{
		...
		{
			globalImages->glowFramebufferImage8[curBuffer]->Bind();
			globalFramebuffers->glowFramebuffer8[nextBuffer]->Bind();
		}
		...
		screenCorrectionParm[0] = textureSizes[curBuffer][2] * sx;
		screenCorrectionParm[1] = textureSizes[curBuffer][3] * sy;
		screenCorrectionParm[2] = textureSizes[curBuffer][4] * sx;
		screenCorrectionParm[3] = textureSizes[curBuffer][5] * sy;
		SetFragmentParm( RENDERPARM_SCREENCORRECTIONFACTOR, screenCorrectionParm ); // rpScreenCorrectionFactor
		SetFragmentParm( RENDERPARM_DIFFUSEMODIFIER, screenBlurStep ); // rpDiffuseModifier
		RB_DrawElementsWithCounters( &backEnd.unitSquareSurface );
		GL_CheckErrors();
	}
