...
int idRenderWorldLocal::BoundsInAreas( const idBounds& bounds, int* areas, int maxAreas ) const
{
	...
	assert( bounds[1][0] - bounds[0][0] < 0x20000 && bounds[1][1] - bounds[0][1] < 0x20000 && bounds[1][2] - bounds[0][2] < 0x20000 ); // motorsep 08-24-2014; expanded bounds so that large worlds don't crash multiplayer
...
void idRenderWorldLocal::GenerateAllInteractions()
{
	...
	int	count = 0;
	int lightDefCount = this->lightDefs.Num();
	int lightDefUpdate = 0;
