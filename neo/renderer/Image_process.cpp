...
{
	// foresthale 2014-05-20: in_p was being advanced incorrectly if width
	// was an odd number, so use a separate row_p variable, we still lose the
	// texel on the right edge in this case but at least it doesn't skew the
	// image data (the higher mipmaps looked sheared/skewed), note that the
	// height is also truncated in non-power-of-two cases
	int		i, j;
	const byte*	in_p, *row_p;
	byte*	out, *out_p;
	int		row;
	int		newWidth, newHeight;
