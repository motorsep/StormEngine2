...
bool R_LoadCubeImages( const char* imgName, cubeFiles_t extensions, byte* pics[6], int* outSize, ID_TIME_T* timestamp )
{
	...
	const char*	cameraSides[6] = { "_right.tga", "_left.tga", "_front.tga", "_back.tga",
									"_up.tga", "_down.tga"
								};
	...
	if( extensions == CF_CAMERA || extensions == CF_CAMERA_ALT )
	{
		sides = cameraSides;
	}
	else
	...
	for( i = 0 ; i < 6 ; i++ )
	{
		...
		else if (pics && extensions == CF_CAMERA_ALT) // motorsep 12-30-2022; to use with cubemaps created from equirectangular panoramas in Bixorama (or perhaps any other similar software)
		{
			switch (i)
			{
			case 0:	// forward
