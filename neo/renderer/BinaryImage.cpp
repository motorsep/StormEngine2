...
void idBinaryImage::Load2DFromMemory( int width, int height, const byte* pic_const, int numLevels, textureFormat_t& textureFormat, textureColor_t& colorFormat, bool gammaMips, bool toolUsage )
{
	...
	for( int level = 0; level < images.Num(); level++ )
	{
		...
		else if( textureFormat == FMT_DEPTH ) // foresthale 2014-02-19: added DEPTH because it was missing
		{
			// DEPTH is composed of 1 uint (however only 24 bits are used)
		...
		else if( textureFormat == FMT_RGBA16F ) // foresthale 2014-02-19: added for HDR view rendering
		{
			// RGBA16F is composed of 4 half-floats
		...
		else if( textureFormat == FMT_DEPTHSTENCIL ) // foresthale 2014-02-19: added for HDR view rendering
		{
			// DEPTHSTENCIL is composed of 1 uint (24 bits of depth and 8 bits of stencil)
			img.Alloc( scaledWidth * scaledHeight * 4 );
			for( int i = 0; i < img.dataSize; i++ )
			{
				img.data[ i ] = pic[ i ];
		...
		else if( textureFormat == FMT_SHADOW_ARRAY ) // foresthale 2014-10-05: this was missing
		{
			// SHADOW_ARRAY is composed of GL_DEPTH_COMPONENT (somewhere between 16 and 32 bits of depth)
			img.Alloc( scaledWidth * scaledHeight * 4 );
			for( int i = 0; i < img.dataSize; i++ )
			{
				img.data[ i ] = pic[ i ];
...
void idBinaryImage::LoadCubeFromMemory( int width, const byte* pics[6], int numLevels, textureFormat_t& textureFormat, textureColor_t& colorFormat, bool gammaMips, bool toolUsage, textureUsage_t usageParm )
{
	...
	fileData.numLevels = numLevels;
	textureUsage_t usage = usageParm;
	...
	{		
		...
		for( int level = 0; level < fileData.numLevels; level++ )
		{
			...
			else // don't worry about anything else, texture should be a multiple of 4
			{
				padSize = scaledWidth;
				padSrc = pic;
			}
			...
			//if( !r_useHightQualitySky.GetBool()) {
				if( image_highQualityCompression.GetBool() && textureFormat == FMT_DXT5 )
					idColorSpace::ConvertRGBToCoCg_Y_GIMPDDS( dxtPic, dxtPic, padSize, padSize );
				else
