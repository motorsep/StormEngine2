...
const char* channelNames[ ANIM_NumAnimChannels ] =
{
...
const char* idAnim::AddFrameCommand( const idDeclModelDef* modelDef, int framenum, idLexer& src, const idDict* def )
{
	...
	{
		return va( "Frame %d out of range(valid range 0-%d)", framenum, anims[ 0 ]->NumFrames() );
	}
	...
	if( token == "call" )
	{
		...
		fc.variant.function = gameLocal.program.FindFunction( token );
		if( !fc.variant.function )
		{
	...
	else if( token == "event" )
	{
		...
		// attempt to convert arguments to their respective types in order to support
		// calling events with arguments
		idList<idEventArg> * args = NULL;

		const int expectedArgs = ev->GetNumArgs();
		if ( expectedArgs > 0 )
		{
			args = new( TAG_ANIM ) idList<idEventArg>();
			args->SetNum( expectedArgs );
			...
			{
				src.ReadTokenOnLine( &argToken );

				switch( argFormat[ i ] )
				{
				case D_EVENT_FLOAT :
					{
						(*args)[ i ] = idEventArg( argToken.GetFloatValue() );
						break;
					}
				case D_EVENT_INTEGER :
					{
						(*args)[ i ] = idEventArg( argToken.GetIntValue() );
						break;
					}
				case D_EVENT_VECTOR :
					{
						idVec3 * vec = new( TAG_ANIM ) idVec3();
						src.Parse1DMatrix( 3, vec->ToFloatPtr() );
						(*args)[ i ] = idEventArg( *vec );
						break;
					}
				case D_EVENT_STRING :
					{
						idStr * str = new( TAG_ANIM ) idStr( argToken );						
						(*args)[ i ] = idEventArg( *str );
						break;
					}
				case D_EVENT_ENTITY:
				case D_EVENT_ENTITY_NULL:
					{
						// ENTITY arguments are stored by string and looked up by name when the event fires
						// because we don't know the lifetime of the entity and it may not exist at load time
						// when the frame commands are parsed. By looking it up as the event fires, it makes it
						// the responsibility of the user if the event does not exist
						idStr * str = new( TAG_ANIM ) idStr( argToken );						
						(*args)[ i ] = idEventArg( *str );
						(*args)[ i ].type = argFormat[ i ];
						break;
					}
				case D_EVENT_TRACE:
					{
						gameLocal.Error( "idAnim.AddFrameCommand : Arg type 'trace' not supported for frame event '%s'.", ev->GetName() );
						break;
					}
				}
			}
		...
		fc.string = new( TAG_ANIM ) idStr( token );
		fc.variant.event.eventDef = ev;
		fc.variant.event.args = args;
	}
	else if( token == "sound" || token == "sound_global" )
	{
		idToken tokenChannel, soundName;
		if( !src.ReadTokenOnLine( &tokenChannel ) || !src.ReadTokenOnLine( &soundName ) )
		{
			return "Unexpected end of line";
		}
		...
		if ( token == "sound" )
			fc.type = FC_SOUND;
		if ( token == "sound_global" )
			fc.type = FC_SOUND_GLOBAL;

		fc.variant.playSound.soundChannel = SND_CHANNEL_ANY;
		...
		const SndChannels channelMap[] = 
		{
			{ "any", SND_CHANNEL_ANY },
			{ "voice1", SND_CHANNEL_VOICE },
			{ "voice2", SND_CHANNEL_VOICE2 },
			{ "body1", SND_CHANNEL_BODY },
			{ "body2", SND_CHANNEL_BODY2 },
			{ "body3", SND_CHANNEL_BODY3 },
			{ "weapon", SND_CHANNEL_WEAPON },
			{ "item", SND_CHANNEL_ITEM },
			{ "heart", SND_CHANNEL_HEART },
		};
		const int mapCount = sizeof(channelMap) / sizeof(channelMap[0]);

		bool foundChannelMap = false;;
		for ( int i = 0; i < mapCount && !foundChannelMap; ++i )
		{
			if ( !idStr::Icmp( channelMap[i].name, tokenChannel.c_str() ) )
			{
				foundChannelMap = true;
				fc.variant.playSound.soundChannel = channelMap[i].channel;
			}
		}
		...
		if ( !foundChannelMap )
		{
			...
			{
				channelNames += channelMap[i].name;
				channelNames += " ";
			}

			return va( "Unknown sound channel '%s'. Valid values are '%s'", tokenChannel.c_str(), channelNames.c_str() );
		}

		if( !soundName.Cmpn( "snd_", 4 ) )
		{
			fc.string = new( TAG_ANIM ) idStr( token );
		}
		else
		{
			fc.variant.playSound.soundShader = declManager->FindSound( soundName );			
			if( fc.variant.playSound.soundShader->GetState() == DS_DEFAULTED )
			{
	...
	else if( token == "skin" )
	{
		...
		if( token == "none" )
		{
			fc.variant.declSkin = NULL;
		}
		else
		{
			fc.variant.declSkin = declManager->FindSkin( token );
			if( !fc.variant.declSkin )
			{
	...
	else if( token == "fx" )
	{
		...
		fc.type = FC_FX;
		fc.variant.declFx = declManager->FindFx( token.c_str() );
		if( !fc.variant.declFx )
		{
	...
	else if( token == "triggerSmokeParticle" )
	{
		...
		fc.variant.declParticle = declManager->FindParticle( token.c_str(), false );
		if( !fc.variant.declParticle )
		{
	...
	else if( token == "melee" )
	{
		...
		fc.type = FC_MELEE;
		fc.variant.declEntity = gameLocal.FindEntityDef( token.c_str(), false );
		if( !fc.variant.declEntity )
		{
	...
	else if( token == "muzzle_flash" )
	{
		...
		fc.variant.jointHandle = jointInfo->num;
	}
	...
	else if ( token == "enableEyeFocus" )
	{
		...
		fc.type = FX_SET_EYE_FOCUS;
		fc.variant.number = atoi( token );
	}
	...
	while( frame != to )
	{
		...
		while( index < end )
		{
			...
			case FC_SCRIPTFUNCTION:
				{
					gameLocal.CallFrameCommand( ent, command.variant.function );
					break;
				}
			...
			case FC_EVENTFUNCTION:
				{
					ent->ProcessEventWithStringArgs( command.variant.event.eventDef, command.variant.event.args );
					break;
				}
			case FC_SOUND:
				{
					if( !command.variant.playSound.soundShader )
					{
						if( !ent->StartSound( command.string->c_str(), command.variant.playSound.soundChannel, 0, false, NULL ) )
						{
					...
					{
						ent->StartSoundShader( command.variant.playSound.soundShader, command.variant.playSound.soundChannel, 0, false, NULL );
					}
					break;
				}
			case FC_SOUND_GLOBAL:
				{
					if( !command.variant.playSound.soundShader )
					{
						if( !ent->StartSound( command.string->c_str(), command.variant.playSound.soundChannel, SSF_GLOBAL, false, NULL ) )
						{
					...
					{
						ent->StartSoundShader( command.variant.playSound.soundShader, command.variant.playSound.soundChannel, SSF_GLOBAL, false, NULL );
					}
					break;
				}
				/*case FC_SOUND_VOICE:
				{
				if( !command.variant.soundShader )
				{
				...
				{
				ent->StartSoundShader( command.variant.soundShader, SND_CHANNEL_VOICE, 0, false, NULL );
				}
				break;
				}
				...
				{
				ent->StartSoundShader( command.variant.soundShader, SND_CHANNEL_VOICE2, 0, false, NULL );
				}
				break;
				}
				case FC_SOUND_BODY:
				...
				{
				ent->StartSoundShader( command.variant.soundShader, SND_CHANNEL_BODY, 0, false, NULL );
				}
				break;
				}
				case FC_SOUND_BODY2:
				...
				{
				ent->StartSoundShader( command.variant.soundShader, SND_CHANNEL_BODY2, 0, false, NULL );
				}
				break;
				}
				case FC_SOUND_BODY3:
				...
				{
				ent->StartSoundShader( command.variant.soundShader, SND_CHANNEL_BODY3, 0, false, NULL );
				}
				break;
				}
			...
			case FC_SKIN:
				{
					ent->SetSkin( command.variant.declSkin );
					break;
				}
			case FC_TRIGGER:
				{
					idEntity* target = gameLocal.FindEntity( command.string->c_str() );
					if( target )
					{
			...
			case FC_TRIGGER_SMOKE_PARTICLE:
				{
					ent->ProcessEvent( &AI_TriggerParticles, command.variant.declParticle->GetName() );
					break;
				}
			case FC_MELEE:
				{
					ent->ProcessEvent( &AI_AttackMelee, command.variant.declEntity->GetName() );
					break;
				}
			case FC_DIRECTDAMAGE:
				{
					ent->ProcessEvent( &AI_DirectDamage, command.variant.declEntity->GetName() );
					break;
				}
			case FC_BEGINATTACK:
				{
					ent->ProcessEvent( &AI_BeginAttack, command.variant.declEntity->GetName() );
					break;
				}
			case FC_ENDATTACK:
				{
			...
			case FC_MUZZLEFLASH:
				{
					ent->ProcessEvent( &AI_MuzzleFlash, modelDef->GetJointName( command.variant.jointHandle ) );
					break;
				}
			case FC_CREATEMISSILE:
				{
					ent->ProcessEvent( &AI_CreateMissile, modelDef->GetJointName( command.variant.jointHandle ) );
					break;
				}
			case FC_LAUNCHMISSILE:
				{
					ent->ProcessEvent( &AI_AttackMissile, modelDef->GetJointName( command.variant.jointHandle ) );
					break;
				}
			case FC_FIREMISSILEATTARGET:
				{
					ent->ProcessEvent( &AI_FireMissileAtTarget, modelDef->GetJointName( command.variant.jointHandle ), command.string->c_str() );
					break;
				}
			case FC_LAUNCH_PROJECTILE:
				{
					ent->ProcessEvent( &AI_LaunchProjectile, command.variant.declEntity->GetName() );
					break;
				}
			case FC_TRIGGER_FX:
				{
					ent->ProcessEvent( &AI_TriggerFX, modelDef->GetJointName( command.variant.jointHandle ), command.string->c_str() );
					break;
				}
			case FC_START_EMITTER:
				{
					const int index = command.string->Find( " " );
					if( index >= 0 )
					{
						idStr name = command.string->Left( index );
						idStr particle = command.string->Right( command.string->Length() - index - 1 );
						ent->ProcessEvent( &AI_StartEmitter, name.c_str(), modelDef->GetJointName( command.variant.jointHandle ), particle.c_str() );
					}
				}				
			...
			case FX_SET_EYE_FOCUS:
				{
					if ( command.variant.number )
			...
			case FC_ENABLE_LEG_IK:
				{
					ent->ProcessEvent( &EV_EnableLegIK, command.variant.number );
					break;
				}
			case FC_DISABLE_LEG_IK:
				{
					ent->ProcessEvent( &EV_DisableLegIK, command.variant.number );
					break;
				}
			case FC_RECORDDEMO:
				{
...
bool idDeclModelDef::Parse( const char* text, const int textLength, bool allowBinaryVersion )
{
	...
	while( 1 )
	{
		...
		if( token == "inherit" )
		{
			...
			const idDeclModelDef* copy = declManager->FindModelDef( token2.c_str(), false );
			if( !copy )
			{
...
idRenderModel* idAnimator::SetModel( const char* modelname )
{
	...
	modelDef = declManager->FindModelDef( modelname, false );
	if( !modelDef )
	{
		return NULL;
	}
...
const idDeclModelDef* ANIM_GetModelDefFromEntityDef( const idDict* args )
{
	...
	modelDef = declManager->FindModelDef( name, false );
