...
void idGrabber::StartDrag( idEntity* grabEnt, int id )
{
	...
	oldImpulseSequence = thePlayer->GetUserCmd()->impulseSequence;
	...
	drag.SetPhysics( phys, clipModelId, viewPos + localPlayerPoint * viewAxis );
	
	// start the screen warp
	thePlayer->PostEventMS( &EV_Player_StartWarp, 0, phys->GetOrigin(), SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, 160, 2000 );
}
...
void idGrabber::StopDrag( bool dropOnly )
{
	...
	if( dragEnt.IsValid() )
	{
		...
		else
		{
			...
			{
				idEntity* player = owner.GetEntity();
	...
	if( warpId != -1 )
	{
		thePlayer->PostEventMS( &EV_Player_FreeWarp, 0, warpId );
		warpId = -1;
	}
...
void idGrabber::UpdateBeams()
{
	...
	{	
		...
		muzzle_joint = thePlayer->GetCurrentWeapon()->GetAnimator()->GetJointHandle( "particle_upper" );
		if( muzzle_joint != INVALID_JOINT )
		{
			thePlayer->GetCurrentWeapon()->GetJointWorldTransform( muzzle_joint, gameLocal.time, muzzle_origin, muzzle_axis );
		}
		else
		{
