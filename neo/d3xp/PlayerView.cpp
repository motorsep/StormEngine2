...
void idPlayerView::SingleView( const renderView_t* view, idMenuHandler_HUD* hudManager )
{
	...
	// draw screen blobs
	if( !pm_thirdPerson.GetBool() ) // ####### SR - && !g_skipViewEffects.GetBool()  fix1
	{
		if( !player->spectating && !g_skipViewEffects.GetBool() ) // ###### SR + && !g_skipViewEffects.GetBool()  fix2
		{
			for( int i = 0 ; i < MAX_SCREEN_BLOBS ; i++ )
			{
...
{
	material = declManager->FindMaterial( "textures/phaeton/warp" );
...
// motorsep 03-21-2015; Underwater view warping code begins; cudos to Jeremy and Forest for helping me conquring C++ :)
...
{
	material = declManager->FindMaterial( "textures/phaeton/enviro_suit" );
}
...
{
	material = declManager->FindMaterial( "textures/phaeton/doubleVision" );
}
...
{
	drawMaterial		= declManager->FindMaterial( "textures/phaeton/bloom2/draw" );
	initMaterial		= declManager->FindMaterial( "textures/phaeton/bloom2/init" );
...
void FullscreenFXManager::Initialize( idPlayerView* pv )
{
	// set the playerview
	playerView = pv;
	blendBackMaterial = declManager->FindMaterial( "textures/phaeton/blendBack" );
	
	// allocate the fx
	CreateFX( "helltime", "helltime", 1000 );
	CreateFX( "warp", "warp", 0 );
	CreateFX( "underwater", "underwater", 2000 ); // motorsep 03-21-2015; Underwater view warping; 2 sec before fading back to normal view
