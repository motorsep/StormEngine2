...
void idGameLocal::CacheDictionaryMedia( const idDict* dict )
{
	...
	while ( kv != NULL )
	{
		...
		kv = dict->MatchPrefix( "table2d", kv );
	}
}
...
{
	const function_t* func = ent->scriptObject.GetFunction( frameCommand );
	if( !func )
	{
...
void idGameLocal::RunDebugInfo()
{
	...
	{
		collisionModelManager->DrawModel( 0, vec3_origin, mat3_identity, origin, g_maxShowDistanceWorld.GetFloat() ); // motorsep 08-27-2015; tune distance at which world bboxes begin to show
	}
...
================
*/
idEntity* idGameLocal::SpawnEntityType( const idTypeInfo& classdef, const idDict* args )
{
	idClass* obj;
	...
	if( common->IsClient() )
	{
		assert( mClientSnapshotRefCount > 0 || mSpawningMapEntities );
	}
#endif
...
=============
*/
void idGameLocal::AddBeam( const beam_t& beam )
{
	beams.Append( beam );
}
...
{
	return ent ? ( ( gameLocal.spawnIds[ ent->entityNumber ] << GENTITYNUM_BITS ) | ent->entityNumber ) : 0;
}
