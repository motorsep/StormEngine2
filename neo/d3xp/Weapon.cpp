...
const idEventDef EV_Weapon_EjectReloadBrass( "ejectReloadBrass" );
const idEventDef EV_Weapon_Start_Attack( "Start_Attack", "d" );
const idEventDef EV_Weapon_Stop_Attack( "Stop_Attack" );
const idEventDef EV_Weapon_NumProjectiles( "NumProjectiles", NULL, 'f' );
const idEventDef EV_Weapon_Spread( "Spread", NULL, 'f' );
const idEventDef EV_Weapon_MeleeAttackLeft( "meleeAttackLeft", "f" );
const idEventDef EV_Weapon_MeleeAttackRight( "meleeAttackRight", "f" );
const idEventDef EV_Weapon_GetBarrelJoint( "getBarrelJoint", "d" );
...
void idWeapon::Spawn()
{
	...
	weaponThread = new idThread( va( "%s_weapon", GetName() ) );
	weaponThread->ManualDelete();
	weaponThread->ManualControl();
}
...
void idWeapon::Save( idSaveGame* savefile ) const
{
	...
	savefile->WriteInt( ents_hit );
	savefile->WriteFloat( decAngle );
...
void idWeapon::Restore( idRestoreGame* savefile )
{
	...
	meleeDefL = gameLocal.FindEntityDef( weaponDef->dict.GetString( "def_melee_left" ), false );
	...
	savefile->ReadInt( ents_hit );
	savefile->ReadFloat( decAngle );
	
	savefile->ReadJoint( ejectReloadJointView );
		...
		if ( ent ) {
			...
			if ( !EntHit( ent->entityNumber ) ) {
				...
				if ( ent->fl.takedamage ) {
					...
					ent->Damage( owner, owner, globalKickDir, meleeDefName2, owner->PowerUpModifier( MELEE_DAMAGE ), tr.c.id );
...
void idWeapon::InitWorldModel( const idDeclEntityDef* def )
{
	...
	if( model[0] && attach[0] )
	{
		...
		ent->BindToJoint( owner, attach, BFL_ORIENTED );
...
void idWeapon::GetWeaponDef(const char* objectname, int ammoinclip) {
	...
	meleeDefLName = weaponDef->dict.GetString( "def_melee_left" );
	if( meleeDefLName.Length() )
	{
		meleeDefL = gameLocal.FindEntityDef( meleeDefLName, false );
		if( !meleeDefL )
		{
			gameLocal.Error( "Unknown melee '%s'", meleeDefLName.c_str() );
		}
	}
...
void idWeapon::SetState( const char* statename, int blendFrames )
{
	...
	weaponThread->CallFunction( this, func, true );
	state = statename;
...
idThread* idWeapon::ConstructScriptObject()
{
	...
	weaponThread->CallFunction( this, constructor, true );
	weaponThread->Execute();
...
void idWeapon::DeconstructScriptObject()
{
	...
	if( destructor )
	{
		...
		weaponThread->CallFunction( this, destructor, true );
		weaponThread->Execute();
		weaponThread->EndThread();
	}
...
void idWeapon::UpdateScript()
{
	...
	count = 10;
	while( ( weaponThread->Execute() || idealState.Length() ) && count-- )
	{
...
void idWeapon::AlertMonsters()
{
	...
	if( tr.fraction < 1.0f )
	{
		...
		else if( ent->IsType( idTrigger::Type ) )
		{
			...
			ent->ProcessEvent( &EV_Touch, owner.GetEntity(), &tr );
		}
	}
...
int idWeapon::AmmoCount() const
{
	...
	{
		return owner->GetInventory()->HasAmmo( ammoType, 1 );
	}
	else
	{
		return 1000;
...
void idWeapon::Event_WeaponRising()
{
	...
	owner->WeaponRisingCallback( this );
}
...
void idWeapon::Event_WeaponLowering()
{
	...
	owner->WeaponLoweringCallback( this );
}
...
void idWeapon::Event_CreateProjectile()
{
	if( !common->IsClient() )
	{
		...
		if( projectileEnt )
		{
			...
			projectileEnt->Bind( owner, BFL_NONE );
			projectileEnt->Hide();
		}
...
void idWeapon::Event_LaunchProjectiles( int num_projectiles, float spread, float fuseOffset, float launchPower, float dmgPower )
{
	...
	if( common->IsServer() || owner->IsLocallyControlled() )
	{
		...
		if( clipSize == 0 && owner->GetInventory() )
		{
			...
			//use it now.
			owner->GetInventory()->UseAmmo( ammoType, ( powerAmmo ) ? dmgPower : ammoRequired );
		}
	...
	{	
		...
		for( i = 0; i < num_projectiles; i++ )
		{
			...
			{
				int serverTimeOnClient = owner->GetUserCmd()->serverGameMilliseconds;
			...
			{
				// Normal launch
				proj->Launch( muzzle_pos, dir, pushVelocity, fuseOffset, launchPower, dmgPower, i );	// ########## SR added i (projNum)
			}
		}
...
void idWeapon::Event_LaunchProjectilesEllipse( int num_projectiles, float spreada, float spreadb, float fuseOffset, float power )
{
	...
	if( !common->IsClient() )
	{
		...
		if( clipSize == 0 && owner->GetInventory() )
		{
			...
			owner->GetInventory()->UseAmmo( ammoType, ammoRequired );
		}
...
void idWeapon::Event_LaunchPowerup( const char* powerup, float duration, int useAmmo )
{
	...
	if( useAmmo && owner->GetInventory() )
	{
		int ammoAvail = owner->GetInventory()->HasAmmo( ammoType, ammoRequired );
		if( !ammoAvail )
		{
			return;
		}
			...
			if( weaponDef->dict.GetBool( "impact_damage_effect" ) )
			{
				...
				else
				{
					...
					if( !( IsType( idPlayer::Type ) && !common->IsMultiplayer() ) )
					{
						...
						if( *decal != '\0' )
						{
							gameLocal.ProjectDecal( tr.c.point, -tr.c.normal, 8.0f, true,  meleeDef->dict.GetFloat( "decal_size", "6.0" ), decal, DEG2RAD( meleeDef->dict.GetFloat( "decal_angle", "0" ) ) ); // SS2 fix; added configurable decal size ; configurable decal angle (somehow it needs to be set to 180 degrees to appear as original image)
						}
					}
					// ====== SS2 fix ends
					...
					if( gameLocal.time > nextStrikeFx )
					{
						...
						{
							gameLocal.ProjectDecal( tr.c.point, -tr.c.normal, 8.0f, true, weaponDef->dict.GetFloat( "decal_size", "6.0" ), decal, DEG2RAD( weaponDef->dict.GetFloat( "decal_angle", "0" ) ) ); // SS2 fix; added configurable decal size ; configurable decal angle (somehow it needs to be set to 180 degrees to appear as original image)
						}
...
/*
================
idWeapon::Event_EjectReloadBrass
