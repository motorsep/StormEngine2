...
EVENT( EV_GetCenter,			idEntity::Event_GetCenter )
...
EVENT( EV_BecomeNonSolid,		idEntity::Event_BecomeNonSolid )
EVENT( EV_SetPowerupTime,		idEntity::Event_SetPowerupTime )
EVENT( EV_IsPowerupActive,		idEntity::Event_IsPowerupActive )
...
================
*/
void idEntity::Bind( idEntity* master, BindFlags flags )
{
	if( !InitBind( master ) )
	{
		return;
...
================
*/
void idEntity::BindToJoint( idEntity* master, const char* jointname, BindFlags flags )
{
...
================
*/
void idEntity::BindToJoint( idEntity* master, jointHandle_t jointnum, BindFlags flags )
{
	if( !InitBind( master ) )
	{
		return;
...
================
*/
void idEntity::BindToBody( idEntity* master, int bodyId, BindFlags flags )
{
	if( !InitBind( master ) )
	{
		return;
...
callback function for when entity fires a weapon of its own
================
*/
void idEntity::WeaponFireFeedback( const idDict* weaponDef )
{
	// implemented in subclasses
}
...
idThread* idEntity::ConstructScriptObject()
{
	...
	if( constructor )
	{
		...
		scriptThread = new idThread( va( "%s_%s", GetName(), constructor->Name() ) );
...
void idEntity::DeconstructScriptObject()
{
	...
	if( destructor )
	{
		...
		thread = new idThread( va( "%s_%s", GetName(), destructor->Name() ) );
		...
		if( parent )
		{
			...
			if( spawnArgs.GetString( "bindToJoint", "", &joint ) && *joint )
			{
				...
				if( ( parent->spawnArgs.GetString( "bindAnim", "", &bindanim ) || parent->spawnArgs.GetString( "anim", "", &bindanim ) ) && *bindanim )
				{
					...
					BindToJoint( parent, joint, flags );
...
/*
================
idEntity::Event_GetCenter
================
*/
void idEntity::Event_GetCenter()
{
	idThread::ReturnVector( GetPhysics()->GetBounds().GetCenter() );
}
...
/*
================
idEntity::Event_PrecacheSkin
================
*/
void idEntity::Event_PrecacheSkin( const char* skinName )
{
	declManager->FindSkin( skinName );
}

/*
================
idEntity::Event_PrecacheParticle
================
*/
void idEntity::Event_PrecacheParticle( const char* particleName )
{
	// Dandel 12-26-2014; allows precaching of Particles
...
/*
================
idEntity::Event_CanBecomeSolid
================
*/
void idEntity::Event_CanBecomeSolid()
{
	idThread::ReturnFloat( CanBecomeSolid() ? 1.0f : 0.0f );
}

/*
================
idEntity::Event_BecomeSolid
================
*/
void idEntity::Event_BecomeSolid()
{
	BecomeSolid();
}
...
/*
================
idEntity::Event_BecomeNonSolid
================
*/
void idEntity::Event_BecomeNonSolid()
{
	BecomeNonSolid();
}
...
/*
==================
idEntity::Event_SetPowerupTime
==================
*/
void idEntity::Event_SetPowerupTime( int powerup, int time )
{
	if( time > 0 )
	{
...
/*
==================
idEntity::Event_IsPowerupActive
==================
*/
void idEntity::Event_IsPowerupActive( int powerup )
{
...
/*
==================
idEntity::Event_WeaponAvailable
==================
*/
void idEntity::Event_WeaponAvailable( const char* name )
{
...
/*
==================
idEntity::Event_GetWeaponEntity
==================
*/
void idEntity::Event_GetWeaponEntity()
{
	idThread::ReturnEntity( GetCurrentWeapon() );
}

/*
=====================
idEntity::Event_RandomPath
=====================
*/
...
{
	msg.WriteBits( bindMaster ? bindMaster->entityNumber : ENTITYNUM_NONE, GENTITYNUM_BITS ); // Bind Entity Number
	msg.WriteBool( bindMaster && fl.bindOrientated ); // Bind Oriented
	msg.WriteBool( bindMaster && fl.bindSnapXform ); // Bind Snap Xform
	if( bindJoint != INVALID_JOINT )
	{
		msg.WriteBits( 1, 2 ); // id 1 = joint bind
		msg.WriteBits( bindJoint, 7 );
	}
	else if( bindBody != -1 )
	{
		msg.WriteBits( 2, 2 ); // id 2 = body bind
		msg.WriteBits( bindBody, 7 );
	}
	else
	{
		msg.WriteBits( 0, 2 ); // id 0 = entity bind
		msg.WriteBits( 0, 7 );
	}
}
...
========================
*/
void idEntity::GetViewPos( idVec3& origin, idMat3& axis ) const
{
...
/*
========================
idEntity::CalculateViewWeaponPos
========================
*/
void idEntity::CalculateViewWeaponPos( idVec3& origin, idMat3& axis ) const
{
	origin = GetPhysics()->GetOrigin();
	axis = GetPhysics()->GetAxis();
}
