...
static const int MAX_MENU_OPTIONS = 5; // default is 6, but Multiplayer menu is disabled for now
...
//#endif
	...
	if( nextState != state )
	{
		...
		if( nextState == SHELL_STATE_PRESS_START )
		{
			...
			nextScreen = SHELL_AREA_ROOT; // motorsep 12-21-2014; go to root menu
...
#define BIND_SHELL_SCREEN( screenId, className, menuHandler )				\
	...
	{
		// BIND_SHELL_SCREEN( SHELL_AREA_START, idMenuScreen_Shell_PressStart, this ); // motorsep 12-21-2014; we do not need Start menu
	...
	menuBar->SetButtonSpacing( 25.0f ); // SS2 fix 45.0f is BFG's default
	...
	soundShader = declManager->FindSound( "gui/list_scroll", true ); // scroll to a new option in a list; currently not utilized
	if( soundShader != NULL )
	{
		sounds[ GUI_SOUND_SCROLL ] = soundShader->GetName();
	}
	soundShader = declManager->FindSound( "gui/btn_PDA_advance", true ); // select an option that would advance to new screen
	if( soundShader != NULL )
	{
		sounds[ GUI_SOUND_ADVANCE ] = soundShader->GetName();
	}
	soundShader = declManager->FindSound( "gui/btn_PDA_back", true ); // select an option that would advance to new screen (previous)
	if( soundShader != NULL )
	{
		sounds[ GUI_SOUND_BACK ] = soundShader->GetName();
	}
	soundShader = declManager->FindSound( "gui/menu_build_on", true ); // when a new menu slides on; currently not utilized
	if( soundShader != NULL )
	{
		sounds[ GUI_SOUND_BUILD_ON ] = soundShader->GetName();
	}
	soundShader = declManager->FindSound( "gui/pda_next_tab", true ); // switching to a new tab in the pda; seems like it's click action sound
	if( soundShader != NULL )
	{
		sounds[ GUI_SOUND_BUILD_ON ] = soundShader->GetName();
	}
	soundShader = declManager->FindSound( "gui/btn_set_focus", true ); // roll over a button with gamepad/kb
	if( soundShader != NULL )
	{
		sounds[ GUI_SOUND_FOCUS ] = soundShader->GetName();
	}
	soundShader = declManager->FindSound( "gui/btn_roll_over", true ); // roll over a button with mouse
	if( soundShader != NULL )
	{
...
void idMenuHandler_Shell::SetupPCOptions()
{
	...
	if( GetPlatform() == 2 && menuBar != NULL )
	{
		...
		else
		{
			...
			navOptions.Append( "#str_swf_dev" );	// DEV
#endif
