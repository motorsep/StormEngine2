...
void idMenuScreen_Shell_Singleplayer::Initialize( idMenuHandler* data )
{
	...
	btnBack->SetLabel( "#str_swf_mainmenu" );
...
void idMenuScreen_Shell_Singleplayer::ShowScreen( const mainMenuTransition_t transitionType )
{
	...
	if( canContinue )
	{
		...
		option.Clear();
		option.Append( "#str_swf_new_game" );	// new game; was #str_01866
		menuOptions.Append( option );
		option.Clear();
		option.Append( "#str_swf_load_game" );	// load game; was #str_01867
		menuOptions.Append( option );
		...
		if( buttonWidget != NULL )
		{
			buttonWidget->SetDescription( "#str_swf_new_desc" ); // was "#str_02209"
		}
		index++;
		buttonWidget = dynamic_cast< idMenuWidget_Button* >( &options->GetChildByIndex( index ) );
		if( buttonWidget != NULL )
		{
			buttonWidget->SetDescription( "#str_swf_load_desc" ); // was "#str_02213"
		}
		index++;
	...
	else
	{
		...
		menuOptions.Append( option );
		option.Clear();
		option.Append( "#str_swf_load_game" );	// load game; #str_01867
		menuOptions.Append( option );
		...
		if( options != NULL )
		{
			...
			if( buttonWidget != NULL )
			{
				buttonWidget->SetDescription( "#str_swf_load_game" ); // #str_02213
			}
		}
	}
...
void idMenuScreen_Shell_Singleplayer::ContinueGame()
{
	...
	if( sortedSaves.Num() > 0 )
	{
		if( sortedSaves[0].damaged )
		{
			...
			optionText.Append( idStrId( "#str_swf_ok" ) );	// OK; was #str_04339
