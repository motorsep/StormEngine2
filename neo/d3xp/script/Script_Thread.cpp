...
const idEventDef EV_Thread_CalcSkyAngles( "calcSkyAngles", "fff", 'v' );
...
/*	's', "key", "Optional string: prefix for spawnarg key match. E.g. \"target\" will match \"target\", \"target1\" etc.",
	...
	'e', "Discover all entities in the map. Returns $null_entity when no more found."); */
...
EVENT( EV_Thread_RandomRange,			idThread::Event_RandomRange )
EVENT( EV_Thread_Wrap,					idThread::Event_Wrap )
EVENT( EV_Thread_Clamp,					idThread::Event_Clamp )
...
EVENT( EV_Thread_CalcSkyAngles,			idThread::Event_CalculateSkyAngles )
...
EVENT( EV_PrecacheSkin,					idThread::Event_PrecacheSkin ) // motorsep 08-23-2014; allows precaching of skins
EVENT( EV_PrecacheParticle,				idThread::Event_PrecacheParticle ) // motorsep 09-16-2015; allows precaching of particles
EVENT( EV_PrecacheModelDef,				idThread::Event_PrecacheModelDef ) // motorsep 09-16-2015; allows precaching of models/anims
...
void idThread::CallFunction( idEntity* self, const function_t* func, bool clearStack )
{
	assert( self );
	ClearWaitFor();
	SetThreadName( va( "%s_%s", self->GetName(), func->Name() ) );
...
void idThread::Event_GetEntity( const char* name )
{
	...
	if( name[ 0 ] == '*' )
	{
		...
		if( ( entnum < 0 ) || ( entnum >= MAX_GENTITIES ) )
		{
			...
			idThread::ReturnString( "mismatch" ); // motorsep 08-31-2014; returns string "mismatch" if quering entity's name mismatches specified string
			return;
		}
...
/*
================
idThread::Event_Floor
================
*/
void idThread::Event_GetFloor( float number )
{
	ReturnInt( idMath::Floor( number ) );
}
...
{
	gameRenderWorld->DebugLine( idVec4( color.x, color.y, color.z, 1.0f ), start, end, SEC2MS( lifetime ) );
}
...
{
	gameRenderWorld->DebugArrow( idVec4( color.x, color.y, color.z, 1.0f ), start, end, size, SEC2MS( lifetime ) );
}
...
{
	gameRenderWorld->DebugCircle( idVec4( color.x, color.y, color.z, 1.0f ), origin, dir, radius, numSteps, SEC2MS( lifetime ) );
}
...
{
	gameRenderWorld->DebugBounds( idVec4( color.x, color.y, color.z, 1.0f ), idBounds( mins, maxs ), vec3_origin, SEC2MS( lifetime ) );
}
...
/*
================
idThread::Event_PrecacheSkin
================
*/
void idThread::Event_PrecacheSkin( const char* skinName )
{
	declManager->FindSkin( skinName );
}


// motorsep 09-16-2015; allows precaching of particles
/*
================
...
// motorsep 09-16-2015; allows precaching of models/anims
/*
================
...
// motorsep 08-23-2014; from TDM (SteveL #3802)
