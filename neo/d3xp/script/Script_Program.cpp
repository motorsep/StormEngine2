...
{
	const function_t* func = GetFunction( "init" );
	return func;
}
...
idVarDef* idProgram::AllocDef( idTypeDef* type, const char* name, idVarDef* scope, bool constant )
{
	...
	else if( scope->TypeDef()->Inherits( &type_object ) )
	{
		...
		// set the value to the variable's position in the object
		def->value.ptrOffset = scope->TypeDef()->Size();
		def->initialized = idVarDef::initializedVariable;
	}
...
============
*/
idVarDef* idProgram::GetDef( const idTypeDef* type, const char* name, const idVarDef* scope, bool checkinherit ) const
{
	idVarDef*		def;
	idVarDef*		bestDef;
	int				bestDepth;
