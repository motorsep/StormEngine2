...
extern const idEventDef EV_Player_FreeWarp;
...
public:
	...
	void					CalculateViewWeaponPos( idVec3& origin, idMat3& axis ) const;
	...
	void					GiveObjective( const char* title, const char* text, const idMaterial* screenshot, idStr target ); // #### SR + idStr target
	...
	//void					StealWeapon( idEntity* player );
	...
	idPhysics_Player*		GetPlayerPhysics();
...
private:
	...
	float 					grapple_disconnect_distance;
	...
	idEntityPtr<idPlayer>				focusPlayer;
	idEntityPtr<idDoor>					focusDoor;
	...
	void					Event_HasInventoryItem( const char* name );
	
	void					Event_GetIdealWeapon();
	void					Event_StartWarp( const idVec3& org, float centerx, float centery, float initialRadius, float durationMsec );
	void					Event_FreeWarp( int warpId );
...
ID_INLINE idPhysics_Player* idPlayer::GetPlayerPhysics()
{
	return &physicsObj;
}
