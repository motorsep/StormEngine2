...
public:
	...
	const function_t*		GetFirstScriptFunction( const char* funcname[], size_t count );
...
private:
	...
	void					Event_SetTeam( float _team );
	void					Event_GetTeam();
	
	bool					InternalPlayAnim( int channel, const int animNum );
	bool					InternalIdleAnim( int channel, const int animNum );
};
