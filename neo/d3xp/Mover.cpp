...
================
*/
void idMover::Swing( float speed, float phase, idAngles& depth )
{
...
/*
================
idMover::Stop_Swinging
================
*/
void idMover::Stop_Swinging()
{
...
void idMover_Binary::SetMoverState( moverState_t newstate, int time )
{
	...
	if( moveMaster == this && moverState != newstate )
	{
		...
		while( kv )
		{
			...
			kv = spawnArgs.MatchPrefix( search, kv );
		}
	}
#endif
...
void idMover_Binary::BindTeam( idEntity* bindTo )
{
	...
	for( slave = this; slave != NULL; slave = slave->activateChain )
	{
		slave->Bind( bindTo, BFL_ORIENTED );
	}
}
